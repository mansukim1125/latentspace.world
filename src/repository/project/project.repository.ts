...
const projects: IProject[] = [
  {
    ...
    achievements: [
      ...
      '캐시톡 Active Users 85% 증가, 신규 방문자 295% 증가',
      '온보딩 전환율 30.35% 달성',
      '사전 인구 정보 기반 메시지 발송 기능 개발로 대규모 공수 절감'
    ],
    content: '## 작업 내용\n' +
      '\n' +
      ...
      '## 조직의 목표\n' +
      '\n' +
      '이 프로젝트를 통해 조직이 달성하고자 하는 목표는 다음과 같습니다.\n' +
      '\n' +
      '- [캐시워크](https://www.notion.so/for-Business-1f209aac666780948f34f52b2dfaac2c?pvs=4) for Business 채널 사업을 통한 외부 업체와의 계약을 통한 수익 창출\n' +
      '- [캐시워크](https://www.notion.so/for-Business-1f209aac666780948f34f52b2dfaac2c?pvs=4) 고객사들의 상품을 홍보하기 위한 채널로써 활용\n' +
      '- [웰컴 메시지](https://www.notion.so/for-Business-1f209aac666780948f34f52b2dfaac2c?pvs=4) 발송을 통해 [캐시워크](https://www.notion.so/for-Business-1f209aac666780948f34f52b2dfaac2c?pvs=4) 유저가 [캐시톡](https://www.notion.so/for-Business-1f209aac666780948f34f52b2dfaac2c?pvs=4) 유저로 전환되는 비율 증가\n' +
      '\n' +
      '## 목표 달성 여부\n' +
      '\n' +
  ...
  },
  {
    ...
    "title": "행운 캐시 보내기: 단순한 캐시를 선물하는 기능에서 랜덤 캐시 뽑기 기능으로의 진화",
    ...
    content:
      '## 프로젝트 개요\n' +
      '\n' +
      '기존 캐시워크 앱의 친구 간 30 캐시 고정 보상 시스템을 완전히 재설계하여, 랜덤 보상과 시간 제한이 결합된 \'행운 캐시\' 시스템으로 개편했습니다. \n\n이 프로젝트에서 DynamoDB 설계부터 대용량 트래픽을 견디는 백엔드 아키텍처를 구현하였습니다.' +
      '\n\n' +
  ...
  },
  {
    ...
    "companyId": "nudge-healthcare",
    "achievements": [
      "일일 API 호출 약 390,000회 → 17,587회 (95.5% 감소)",
      "월간 API 호출 3,793,832회 → 597,965회 (84.2% 감소)",
      "API 쿼터 초과 오류 100% 해소",
      "카카오톡 공유 기능 정상화로 서비스 안정성 확보",
    ...
    "content":
      ...
      '### 발견된 문제\n' +
      '\n' +
      '2024년 1월 11일부터 서비스 사용자들로부터 카카오톡 공유 기능이 동작하지 않는다는 보고가 접수되었습니다. 기능 장애는 다음과 같은 영역에서 발생했습니다.\n' +
      '\n' +
      '- 캐시톡 SNS 연동 > 카카오 연동하기\n' +
      '- 친구 초대 > 카카오톡으로 공유하기\n' +
      '- 앱 전반의 카카오톡 공유 기능\n' +
      '\n' +
      '로그 분석 결과, 카카오 개발자 콘솔에서 `/v1/friends`API (카카오 측 API) 에서 대량의 오류가 발생한 것을 확인했습니다.\n' +
      '![카카오 연동하기{200x}](https://footage.latentspace.world/production/images/%E1%84%8B%E1%85%B5%E1%84%86%E1%85%B5%E1%84%8C%E1%85%B5.jpeg)' +
      '![카카오톡으로 공유하기{200x}](https://footage.latentspace.world/production/images/IMG_5024.png)' +
      '\n' +
      '### 원인 분석\n' +
      '\n' +
      ...
      '   - 일일 약 39만 회의 카카오 친구 목록 API를 호출하는 상황이었습니다. (45,000명 × 2회 × 4.4회)\n' +
      '\n' +
      '## 어떻게 해결했는가?\n' +
      '\n' +
      '문제 해결을 위해 다양한 이해관계자(PM, 서버 개발자, 클라이언트 개발자, 웹뷰 개발자)와 협업하여 해결 방안을 도출하였습니다.\n' +
      '\n' +
      '1. **서버 측 카카오 API 응답 캐싱**\n' +
      '   - 사용자별 카카오 친구 목록을 서버에 캐싱\n' +
      '   - 캐싱 주기: 24시간 (조정 가능)\n' +
      ...
      '   - 유효한 캐시 없을 경우, 카카오 API에서 최대 100명까지 친구 목록 조회 후 캐싱\n' +
      '   - 유효한 캐시 있을 경우, 캐싱된 데이터 반환\n' +
      '\n' +
      '2. 사용자가 친구 목록을 새로고침하면:\n' +
      '   - 이미 로드된 친구 수 확인\n' +
      '   - 아직 로드되지 않은 친구가 있는 경우, 다음 100명을 카카오 API에서 조회하여 캐시에 추가\n' +
      ...
      '| 항목 | 개선 전 | 개선 후 | 감소율 |\n' +
      '|------|---------|---------|--------|\n' +
      '| 일일 API 호출 수 | ~390,000회 | ~17,587회 | 95.5% |\n' +
      '| 월간 API 호출 수 | 3,793,832회 | ~597,965회 | 84.2% |\n' +
      '| 쿼터 초과 오류 | 빈번 | 없음 | - |\n' +
      '\n' +
      '![개선 이후 카카오 /v1/friend API 호출량{700x}](https://footage.latentspace.world/production/images/%E1%84%80%E1%85%A2%E1%84%89%E1%85%A5%E1%86%AB_%E1%84%80%E1%85%A7%E1%86%AF%E1%84%80%E1%85%AA.png)\n' +
      '### 정성적 성과\n' +
      '\n' +
      '1. **서비스 안정성 향상**\n' +
      '   - 카카오톡 공유 기능 정상화\n' +
  ...
  },
  {
    ...
    content:
      ...
      '- **상태 관리**: Redis Cluster의 PubSub 기능을 활용한 WebSocket 연결 상태 관리\n' +
      '\n' +
      '## 장애 상황과 초기 대응\n' +
      '\n' +
      '### 발견된 증상\n' +
      ...
      '\n' +
      '이 과정에서 **_표면적 증상 완화가 근본 원인 해결을 의미하지 않는다_**는 중요한 교훈을 얻게 되었습니다.\n' +
      ...
      '   - `fetchSockets`는 Redis PubSub을 통해 모든 서버 인스턴스에 Socket 연결 가져오기 요청을 브로드캐스트하고, 각 서버 인스턴스가 보유한 소켓 객체 리스트 응답을 기다립니다.\n' +
      '   - 기존에 어느 서버 인스턴스에 연결되었는지를 알지 못하기 때문입니다.\n' +
      '\n' +
